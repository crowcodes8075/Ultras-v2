...
public class UltraGramiel
{
    ...
    public List<IOption> Options = new()
    {
        new Option<string>(
            "Guide",
            "Guide",
            "You have to manually equip the classes that will taunt, \nthis is the only Ultra Boss Script that does not use custom \npreset classes because Scroll of Enrage gets bugged \nand cannot be used even if you try manually.",
            "Click Me And Read Below"
        ),
    ...
    private void ExecuteCrystalTaunt(int targetCrystalMapId)
    {
        ...
        int attempts = 0;
        while (!Bot.ShouldExit && attempts < 120)
        {
            if (!Bot.Player.Alive)
                break;
