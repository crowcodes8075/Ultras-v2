...
public class ChampionDrakath
{
    ...
    public List<IOption> Options = new()
    {
        ...
        new Option<int>("ThresholdBuffer", "Threshold Buffer (HP)", "How far above each HP threshold to start taunting. Increase if your team nukes fast and misses thresholds. Default: 150000", 150000),CoreBots.Instance.SkipOptions,
    };
    bool SoloTaunt;
        ...
        while (!Bot.ShouldExit)
        {
                ...
                && Bot.Player.Target?.HP > 0)
            {
                ...
                for (int i = 0; i < thresholds.Length; i++)
                {
                    ...
                    for (int p = 0; p < 20 && !Bot.ShouldExit; p++)
                    {
                        if (!Bot.Player.Alive)
                        {
                            Bot.Wait.ForTrue(() => Bot.Player.Alive, 20);
